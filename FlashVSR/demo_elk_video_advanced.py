...
def init_pipeline():
    ...
    if PARA_ATTN_AVAILABLE:
        ...
        try:
            # Apply first block cache for memory efficiency with aggressive settings
            apply_cache_on_pipe(
                pipe,
                residual_diff_threshold=0.0,  # More aggressive caching
            )
            print("✅ First block cache applied (aggressive mode)")
...
def main():
    ...
    # Use the elk_384p.mp4 video from the workspace (matches official examples)
    input_video = "/workspace/vidaio-win/elk_384p.mp4"
    ...
    sparse_ratio = 1.5      # Use 1.5 for faster processing (less stable but faster)
    
    # Try with fewer frames for maximum memory efficiency
    max_frames = 24  # Balanced frame limiting for processing
    
    try:
        print("Initializing FlashVSR pipeline...")
        pipe = init_pipeline()
        
        # Clear GPU cache before processing (as in official script)
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        ...
        print(f"Processing video with FlashVSR (Advanced Memory Optimization)...")
        print(f"  - Input resolution: {th}x{tw}")
        print(f"  - Number of frames: {F}")
        print(f"  - Scale factor: {scale}x")
        print(f"  - Sparse ratio: {sparse_ratio} (faster mode)")
        ...
        video = pipe(
            ...
            kv_ratio=2.0,  # Reduced KV ratio for memory efficiency
            local_range=9,  # Use 9 for sharper details (less memory than 11)
            color_fix = True,
        )
        ...
        output_path = os.path.join(RESULT_ROOT, f"FlashVSR_advanced_elk_seed{seed}.mp4")
        print(f"Saving video to: {output_path}")
        save_video(video, output_path, fps=fps)
        print("✅ FlashVSR advanced processing complete!")
