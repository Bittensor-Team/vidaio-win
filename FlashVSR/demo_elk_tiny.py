...
def main():
    ...
    try:
        ...
        LQ, th, tw, F, fps = prepare_input_tensor(input_video, scale=scale, dtype=dtype, device=device)
        ...
        save_video(video, os.path.join(RESULT_ROOT, f"FlashVSR_Tiny_elk_seed{seed}.mp4"), fps=fps, quality=6)
        print("✅ FlashVSR-Tiny processing complete!")
        print(f"Output video: {os.path.join(RESULT_ROOT, f'FlashVSR_Tiny_elk_seed{seed}.mp4')}")
