...
from examples.WanVSR.utils.TCDecoder import build_tcdecoder
...
def prepare_input_tensor(path: str, scale: float = 4, dtype=torch.bfloat16, device='cuda', max_frames=64):
    ...
    if is_video(path):
        ...
        # Limit frames for memory optimization
...
def init_pipeline():
    ...
    mis = pipe.TCDecoder.load_state_dict(torch.load("./examples/WanVSR/FlashVSR/TCDecoder.ckpt"), strict=False)
...
def main():
    ...
    # Use the elk video from the workspace
    ...
    # Test different frame limits to find the sweet spot
    frame_limits = [32, 48, 64, 80]
    
    for max_frames in frame_limits:
        print(f"\n{'='*80}")
        print(f"Testing FlashVSR-Tiny with {max_frames} frames")
        print(f"{'='*80}")
        
        seed, scale, dtype, device = 0, 2.0, torch.bfloat16, 'cuda'  # Use 2x scale for SD2HD task
        sparse_ratio = 2.0      # Recommended: 1.5 or 2.0. 1.5 → faster; 2.0 → more stable.
        
        try:
            print("Initializing FlashVSR-Tiny pipeline...")
            pipe = init_pipeline()
            ...
            if torch.cuda.is_available():
                ...
                print(f"GPU Memory before processing: {torch.cuda.memory_reserved(0) / 1024**3:.1f} GB reserved, {torch.cuda.memory_allocated(0) / 1024**3:.1f} GB allocated")
            
            print("Preparing input tensor...")
            LQ, th, tw, F, fps = prepare_input_tensor(input_video, scale=scale, dtype=dtype, device=device, max_frames=max_frames)
            
            print(f"Processing video with FlashVSR-Tiny...")
            ...
            save_video(video, os.path.join(RESULT_ROOT, f"FlashVSR_Tiny_elk_{max_frames}frames_seed{seed}.mp4"), fps=fps, quality=6)
            print("✅ FlashVSR-Tiny processing complete!")
            print(f"Output video: {os.path.join(RESULT_ROOT, f'FlashVSR_Tiny_elk_{max_frames}frames_seed{seed}.mp4')}")
            
            # Success! Break out of the loop
            break
            
        except Exception as e:
            print(f"❌ Error with {max_frames} frames: {e}")
            if "CUDA out of memory" in str(e):
                print(f"   → Reducing frame count...")
                continue
            else:
                print(f"   → Other error, stopping tests")
                break
