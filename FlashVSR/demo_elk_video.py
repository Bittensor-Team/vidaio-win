...
def prepare_input_tensor(path: str, scale: float = 4, dtype=torch.bfloat16, device='cuda'):
    ...
    if is_video(path):
        ...
        # Limit frames to reduce memory usage (max 64 frames for 384p)
        max_frames = min(64, total)
        idx = list(range(max_frames)) + [max_frames - 1] * 4
        F = largest_8n1_leq(len(idx))
        if F == 0:
            rdr.close()
            raise RuntimeError(f"Not enough frames after padding in {path}. Got {len(idx)}.")
        idx = idx[:F]
        print(f"[{os.path.basename(path)}] Target Frames (8n-3): {F-4} (limited from {total})")
...
def main():
    ...
    seed, scale, dtype, device = 0, 4, torch.bfloat16, 'cuda'  # Use 4x scale as recommended
    ...
    try:
        ...
        output_path = os.path.join(RESULT_ROOT, f"FlashVSR_elk_seed{seed}.mp4")
