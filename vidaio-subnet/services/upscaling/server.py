...
def upscale_video(payload_video_path: str, task_type: str):
    ...
    try:
        ...
        # SD2HD: Standard Definition (480p) → HD (1080p) = 2x
        # HD24K: HD (1080p) → 4K (2160p) = 2x  
        # SD24K: SD (480p) → 4K (2160p) = 4x
        # 4K28K: 4K (2160p) → 8K (4320p) = 2x
        task_scale_map = {
            "SD2HD": "2",
            "HD24K": "2", 
            "SD24K": "4",
        ...
        if task_type not in task_scale_map:
            logger.warning(f"⚠️  Unknown task_type '{task_type}', defaulting to 2x scale")
        else:
            logger.info(f"✅ Task type: {task_type}, Scale factor: {scale_factor}x")
